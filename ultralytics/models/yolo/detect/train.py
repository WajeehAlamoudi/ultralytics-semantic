...
class _GradCkptWrapper(nn.Module):
    ...
    Uses self.wrapped(x) (i.e. __call__) for inference so AMP dtype handling, model.half(), and model.eval() all
    propagate correctly through the registered child module — none of which work when forward() is replaced by a plain
    function.
    """
